...
const localImages = [
  "/images/daily/2026-08-12/claude-watermark.jpg",
  "/images/daily/2026-08-12/wechat-xiaowei-ai-review.jpg",
];
...
test("2026-08-12 published copies use the repaired local images", () => {
  for (const relativePath of publishedCopies) {
    ...
    for (const imagePath of localImages) {
      assert.match(markdown, new RegExp(imagePath.replaceAll("/", "\\/")));
    }
...
test("repaired local images are non-empty JPEG files", () => {
  for (const publicPath of localImages) {
    const image = fs.readFileSync(path.join(root, "static", publicPath));
