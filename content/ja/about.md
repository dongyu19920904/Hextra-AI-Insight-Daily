---
title: 关于爱窝啦
type: about
sidebar:
  exclude: true
---
## 🏠 Aivora（アイボラ）について

> **すべての中国語ユーザーに、最強AIツールを低コストで体験してもらう**

Aivoraは、AIツールのアカウントサービスに特化したプラットフォームで、中国語ユーザーが世界トップクラスのAIツールを最低コストで使えるよう支援しています。

### 🎯 私たちのミッション

- **ハードルを下げる**：国内ユーザーがAIツールを使う際の決済・登録の難題を解決
- **安定性を確保**：長期的に安定した、信頼できるアカウントサービスを提供
- **究極のコスパ**：より多くの人が先進的なAIツールを手に入れられるように

### ⭐ コアサービス

以下のAIツールのアカウントサービスを提供しています：

#### 🔥 人気商品

**1. Cursor Pro 切り替え器 ⌨️**
- ✨ GPT-5.1、Claude、Gemini の3大モデルに対応
- ✨ 7日間trial、24時間アクティベート、4-6日間稼働
- ✨ 1アカウント10ドル分、自動切り替え
- 💰 **価格**：7日間2アカウント/9.9元 | 5アカウント/21.9元 | 20日間10アカウント/43.9元
- 📖 [利用ドキュメント](https://w1yklj2r7gv.feishu.cn/wiki/FXv7w7oFHixdrRkwQEecjrwKnsc)

**2. Warp 切り替え器 ⚡**
- ✨ 無限おかわり、自動切り替え
- ✨ Claude 4.5 + Gemini 3
- ✨ 1アカウント150クレジット
- 💰 **価格**：日額3.9元 | 週額11.9元 | 半月19.9元 | 月額29.9元
- 📖 [利用ドキュメント](https://w1yklj2r7gv.feishu.cn/wiki/ZgOOwcyAjiPNJ3kmQvkcrvwCnXg)

**3. Augment Pro アクティベーター 💎**
- ✨ 純正公式API経由、シームレス切り替え
- ✨ VSCode、IntelliJ IDEA、JetBrainsシリーズ全対応
- 💰 **価格**：3日間5ドル/7.9元 | 週額10ドル/18.9元 | 半月21ドル/39.9元 | 月額52ドル/79.9元
- 📖 [利用ドキュメント](https://w1yklj2r7gv.feishu.cn/wiki/VqwUw2DIEiY1FikYuSUcCXGPn0c)

**4. ChatGPT ビジネス版 👑**
- ✨ Plus超えの安定性、CodeX、Sora2、深層研究対応
- 💰 **価格**：月額19.9元（公式$20/月）

**5. Claude/Gemini/Codex 3 in 1**
- ✨ 200Kコンテキスト、Opus/Sonnet 4.5
- 💰 **永久クレジット**：20ドル/11.9元 | 50ドル/19.9元 | 100ドル/32.9元 | 200ドル/64.9元
- 💰 **月額・日額**：30ドル/日/129元 | 50ドル/日/179元
- 📖 [利用ドキュメント](https://w1yklj2r7gv.feishu.cn/wiki/WCHuwmjP0iW7YxkwWU5c7dBEnbf)

#### 💎 公式完成品アカウント（手動発送）

- **Cursor Pro Max 専用アカウント**：79元/月（本物のPro、1ヶ月保証）
- **ChatGPT Plus**：完成品アカウント79元/月、代理チャージ139元/月
- **Gemini Plus 年間パス**：59.9元/年（2Tクラウドストレージ付き）
- **Consensus Pro**：AI論文検索、39.9元/年
- **Perplexity Pro**：AI検索エンジン、19.9元/年

> 📞 **完成品アカウント購入はグループオーナーへDM、またはカスタマーサービスWechat：prompt2333までご連絡ください**

### 📰 Aivora AI デイリー

当サイトは、Aivoraのコンテンツサービス部門で、**毎日自動的に世界中のAI最新動向をまとめています**：

- 🔥 業界ホットニュース
- 📦 オープンソースプロジェクト発見
- 📄 最先端学術論文
- 💬 テック系インフルエンサーの見解
- 🚀 製品機能アップデート

すべてのコンテンツは **CloudFlare Workers + AI** により自動収集・生成・公開され、タイムリーかつ網羅的な情報提供を実現しています。

### 🔗 お問い合わせ

- **公式サイト**：[https://aivora.cn](https://aivora.cn)
- **カスタマーサービスWechat**：prompt2333
- **メール**：dongyu199209@outlook.com

---

<div style="text-align: center; margin: 2rem 0;">
<a href="https://aivora.cn?utm_source=about_page" style="display: inline-block; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 12px 32px; border-radius: 8px; text-decoration: none; font-weight: bold; box-shadow: 0 4px 15px rgba(102,126,234,0.4);">🚀 今すぐAIアカウントショップへ</a>
</div>

**Aivora、中国語ユーザーのためにAI世界への扉を開く 🚪✨**